...
public class WheelDatePickerTH extends LinearLayout implements WheelPicker.OnItemSelectedListener,
        IDebug, IWheelPicker, IWheelYearPicker, IWheelMonthPicker,
        IWheelDayPicker, IWheelDatePickerTH {
    private static final SimpleDateFormat SDF =
            new SimpleDateFormat("yyyy-M-d", Locale.getDefault());

    private WheelYearPickerTH mPickerYear;
    private WheelMonthPickerTH mPickerMonth;
    ...
    public WheelDatePickerTH(Context context) {
        this(context, null);
    }
    @SuppressLint("Recycle")
    public WheelDatePickerTH(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        LayoutInflater.from(context).inflate(R.layout.view_wheel_date_picker_th, this);

        mPickerYear = (WheelYearPickerTH) findViewById(R.id.wheel_date_picker_year_th);
        mPickerMonth = (WheelMonthPickerTH) findViewById(R.id.wheel_date_picker_month_th);
...
//        mPickerMonth.setMaximumWidthText("00");
    ...
    public void onItemSelected(WheelPicker picker, Object data, int position) {
        if (picker.getId() == R.id.wheel_date_picker_year_th) {
            mYear = ((int) data) - (locale.getISO3Language().startsWith("th") ? 543:0);
            mPickerDay.setYear(mYear);
        } else if (picker.getId() == R.id.wheel_date_picker_month_th) {
            mMonth = position + 1;
    ...
    @Override
    public WheelYearPickerTH getWheelYearPickerTH() {
        return mPickerYear;
    }

    @Override
    public WheelMonthPickerTH getWheelMonthPickerTH() {
        return mPickerMonth;
    }
    ...
    public interface OnDateSelectedListener {
        void onDateSelected(WheelDatePickerTH picker, Date date);
    }
}